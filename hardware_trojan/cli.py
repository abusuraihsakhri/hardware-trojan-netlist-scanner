"""
Command-Line Interface for Gate-Level Netlist Hardware Trojan & Stealth Payload Scanner.
"""
import argparse
import csv
import json
...
from .agents import TrojanScannerCoordinator

coordinator = TrojanScannerCoordinator()


def main(argv=None):
    parser = argparse.ArgumentParser(prog="hardware-trojan-netlist-scanner", description="Gate-Level Netlist Hardware Trojan & Stealth Payload Scanner")
    ...
    if args.command == "audit":
        ...
        print("=" * 80)
        print(f"  GATE-LEVEL NETLIST HARDWARE TROJAN & STEALTH PAYLOAD SCANNER")
        print(f"  Domain: Hardware Security | Standard: Trust-HUB Hardware Trojan Taxonomy")
    ...
    if args.command == "serve":
        try:
            ...
            app = create_app()
            if app:
                print(f"Starting Gate-Level Netlist Hardware Trojan & Stealth Payload Scanner on http://{args.host}:{args.port}")
